...
export default function ApplyForm({ initialType }: { initialType: string }) {
  ...
  if (state.ok) {
    return (
      <div className="apForm apDone" role="status">
  ...
  return (
    <form action={action} className="apForm">
      {/* 허니팟 — 사람에게는 보이지 않는다 */}
      <input type="text" name="website" tabIndex={-1} autoComplete="off" aria-hidden="true" className="hp" />

      <fieldset className="apSeg">
        <legend>문의 유형</legend>
        {TYPES.map(t => (
          <label key={t}>
            <input type="radio" name="type" value={t} defaultChecked={t === initialType} />
            <span>{t}</span>
          </label>
        ))}
      </fieldset>
      ...
      <div className="apField">
        ...
        <textarea id="ap-message" name="message" required rows={5} maxLength={4000} minLength={10}
      ...
      <div className="apActions">
        <Submit />
        <p className="apPrivacy">남겨주신 정보는 검토와 회신에만 씁니다.</p>
      </div>
    </form>
  );
}
